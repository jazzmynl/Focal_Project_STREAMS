...
psa |> 
  ...
  #code above changes data to presence/absence(only 0s and 1s)
  colSums()
```
...
shan_div <- diversity(psa, index = "shannon", MARGIN = 2)
simp_div <- diversity(psa, index = "simpson", MARGIN = 2)
sprich <- ps |> colSums()

vec <- colnames(ps)

bind_cols(vec, shan_div, simp_div, sprich) |>
  rename(Site = '...1', Shannon = '...2', Simpson = '...3', SpRich = '...4') |>
  separate(Site, into = c("Null", "site", "UpperLower", "Rep"), sep = "")
```
